...
def send_data_request_object(data_request_object):
    ...
    try:
        if (data_request_object['__FrameData__numpy_data'] == False): return 400, 'data_request_object must be a FrameData type'
        ...
        config_params = []
        config_params.append(data_request_object['__FrameData__esp_id'])
        config_params.append(data_request_object['__FrameData__offset'])
        config_params.append(data_request_object['__FrameData__iouti'])
        ...
        if(config_params[2] == 1):
            iouti_boolean = dk.detect(file_name)
            outputM = outputMessage(1,"","","")
            return outputM
        else:
            speech = dc.detect(file_name)
            outputM = outputMessage(0,speech[0],speech[1],speech[2],speech[3])
            return outputM
